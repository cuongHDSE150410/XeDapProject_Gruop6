...
//                    for (Item c : cart) {
//                        if (bf.find(c.getBike().getProductId()) == null) {
//                             cart.remove(c);
//                        }
//                    }
                    for (int i = 0; i < cart.size(); i++) {
                        Item item = cart.get(i); // concurrent modification exception
                        if (bf.find(item.getBike().getProductId()) == null) {
                            cart.remove(i);
                        }
                    }
                }
                HomeController h = new HomeController();
                h.total(request, response);
                break;
            }
            case "clear":
